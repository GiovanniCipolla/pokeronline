...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable() // Disabling csrf
				...
				.antMatchers("/api/tavolo/specialGuest/**").hasRole("SPECIAL_PLAYER")
				
				.antMatchers("/api/tavolo/admin/**").hasRole("ADMIN")

				.antMatchers("/api/tavolo/private/**").hasAnyRole("ADMIN", "SPECIAL_PLAYER")
				
				.antMatchers("/api/utente/autenticato/ricarica/**").authenticated()
